...
TestContext decoder_test_bytewise(TDecoder decoder_prototype, const uint8_t (&encoded)[], size_t length, TVal decoded) {
    ...
    TEST_EQUAL(decoder.process_bytes_({nullptr, 0}, nullptr), kStreamOk);
    for (size_t i = 0; i < length - 1; ++i) {
        TEST_EQUAL(decoder.process_bytes_({encoded + i, 1}, &processed_bytes), kStreamOk);
        TEST_EQUAL(decoder.process_bytes_({encoded + i, 0}, &processed_bytes), kStreamOk);
        TEST_EQUAL(processed_bytes, i + 1);
        TEST_ZERO(decoder.get());
    }
    TEST_EQUAL(decoder.process_bytes_({encoded + length - 1, 1}, &processed_bytes), kStreamClosed);
    TEST_EQUAL(processed_bytes, length);
    TEST_EQUAL(decoder.process_bytes_({encoded + length - 1, 0}, &processed_bytes), kStreamClosed);
    TEST_EQUAL(decoder.process_bytes_({encoded + length - 1, 1}, &processed_bytes), kStreamClosed);
    TEST_EQUAL(processed_bytes, length);
...
TestContext decoder_test_chunkwise(TDecoder decoder_prototype, const uint8_t (&encoded)[], size_t length, TVal decoded) {
    ...
    StreamStatus status = kStreamClosed;
    ...
    while (bufptr.length > 1) {
        ...
        if (status == kStreamOk) {
            ...
            TEST_EQUAL(status, kStreamClosed);
...
TestContext encoder_test_bytewise(TEncoder encoder_prototype, const uint8_t (&encoded)[], size_t length, TVal decoded) {
    ...
    TEST_ASSERT(encoder.get_bytes_({nullptr, 0}, nullptr), kStreamClosed);
    encoder.set(&decoded);

    uint8_t encoded_out[length + 1];
    for (size_t i = 0; i < length - 1; ++i) {
        TEST_EQUAL(encoder.get_bytes_({encoded_out + i, 1}, &generated_bytes), kStreamOk);
        TEST_EQUAL(encoder.get_bytes_({encoded_out + i, 0}, &generated_bytes), kStreamOk);
        TEST_EQUAL(generated_bytes, i + 1);
    }
    TEST_EQUAL(encoder.get_bytes_({encoded_out + length - 1, 1}, &generated_bytes), kStreamClosed);
    TEST_EQUAL(generated_bytes, length);
    TEST_EQUAL(encoder.get_bytes_({encoded_out + length - 1, 0}, &generated_bytes), kStreamClosed);
    TEST_EQUAL(encoder.get_bytes_({encoded_out + length - 1, 1}, &generated_bytes), kStreamClosed);
    TEST_EQUAL(generated_bytes, length);
