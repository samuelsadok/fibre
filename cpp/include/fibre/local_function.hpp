...
/*        template<TRet(*Function)(TInputs...)>
    ...
    class WithStaticFuncPtr2 : public LocalEndpoint {
        ...
        // @brief Returns a JSON snippet that describes this function
        //bool get_as_json(char output[256]) final {
        //    //return FunctionJSONAssembler<TFuncProps, func_props>::get_as_json(output);
        //}
    };
};
...
private:
    ...
    using this_type = FixedIntEncoder;
public:
    //using type = tuple_cat_t<std::tuple<this_type>, typename tail::type>;
    //using remainder = typename tail::remainder;
...
public:
    ...
    static void serialize(StreamSink* output, const char * (&&str), size_t&& length, Ts&&... tail_values) {
        if (str) {
            uint32_t i = length;
            Serializer<uint32_t>::serialize(i, output);
            LOG_FIBRE(SERDES, "will write string len ", length);
            size_t processed_bytes = 0;
